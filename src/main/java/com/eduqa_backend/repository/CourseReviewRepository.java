...
public interface CourseReviewRepository extends JpaRepository<CourseReview, UUID> {
        @Query("""
                        SELECT new com.eduqa_backend.dto.StudentReviewDTO(cr.id,lc.course.name,lc.course.code,lc.user.picture,lc.user.name,lc.user.email,cr.marks) FROM  CourseReview cr LEFT JOIN LectureCourse lc ON cr.lectureCourse.id=lc.id
        ...
        @Query("""
                SELECT AVG(cr.marks) FROM CourseReview cr Where cr.lectureCourse.user.email=:email GROUP BY cr.lectureCourse
                """)
        long countByLectureReview(String email);
}
